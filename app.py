...
with k1:
    st.metric("Temporadas analizadas", len(df_f))
with k2:
    st.metric(
        "Puntos perdidos vs campeón",
        f"{total_gap}",
        delta=f"{avg_gap:.0f} pts/temporada",
        delta_color="inverse",
    )
with k3:
    st.metric("PPG promedio", f"{avg_ppg:.2f}", delta=f"vs {best_ppg:.2f} (Mourinho)")
with k4:
    st.metric("Coste indemnizaciones", f"£{total_comp:.1f}M", delta="10 años", delta_color="off")
with k5:
    st.metric("Títulos Premier", titles, delta="-9 vs Top6 líderes", delta_color="inverse")
with k6:
    st.metric("Posición promedio PL", f"{df_f['position'].mean():.1f}°")
...
with tab1:
    ...
    if benchmark_mode:
        ...
        fig_pts.add_trace(
            go.Scatter(
                ...
                fill="toself",
                fillcolor="rgba(247,129,102,0.1)",
                line=dict(color="rgba(0,0,0,0)"),
                name="Brecha con campeón",
                showlegend=True,
            )
        )
    ...
    if show_trend:
        ...
        fig_pts.add_trace(
            go.Scatter(
                ...
                y=trend_y,
                mode="lines",
                name=f"Tendencia ({slope:+.1f} pts/temporada)",
    ...
    with col_a:
        ...
        fig_gd.add_trace(
            go.Bar(
                ...
                marker_color="rgba(247,129,102,0.7)",
                text=df_f["ga"],
                textposition="inside",
            )
        )
        ...
        fig_gd.update_layout(
            **PLOTLY_THEME,
            title="Goles: a favor / en contra / diferencial",
            height=320,
            barmode="overlay",
        )
    ...
    with col_b:
        fig_pos = go.Figure()
        colors_pos = [
            COLORS["manutd"] if p <= 4 else (COLORS["warn"] if p <= 6 else COLORS["bad"])
        ...
        fig_pos.add_hline(
            y=4,
            line_dash="dash",
            line_color=COLORS["champ"],
            annotation_text="Champions League zone",
            annotation_font_color=COLORS["champ"],
        )
...
with tab2:
    ...
    with col_l:
        ...
        bar_colors = [
            COLORS["manutd"] if ppg >= 1.8 else (COLORS["warn"] if ppg >= 1.6 else COLORS["bad"])
            for ppg in sorted_mgr["ppg"]
        ]
        fig_mgr.add_trace(
            go.Bar(
                y=sorted_mgr["manager_clean"],
                x=sorted_mgr["ppg"],
                orientation="h",
        ...
        fig_mgr.add_vline(
            ...
            line_dash="dot",
            line_color=COLORS["champ"],
            annotation_text="Elite (>2.0 ppg)",
            annotation_font_color=COLORS["champ"],
        )
        fig_mgr.update_layout(
            **PLOTLY_THEME,
            title="Puntos por partido (PPG) — Comparativa de gestiones",
            height=360,
        )
    ...
    display = mgr_df.rename(
        columns={
            "manager_clean": "Entrenador",
            "seasons": "Temporadas",
            "pts_total": "Pts totales",
            "ppg": "PPG",
            "avg_pos": "Pos. media",
            "avg_gf": "GF/temp.",
            "avg_ga": "GA/temp.",
            "total_comp": "Indemniz. (£M)",
        }
    )
    st.dataframe(
        display.style.background_gradient(
            subset=["PPG"], cmap="RdYlGn", vmin=1.2, vmax=2.1
        ).background_gradient(subset=["Indemniz. (£M)"], cmap="Reds"),
        use_container_width=True,
        hide_index=True,
    )
    st.info(
        "💡 **Hallazgo clave:** Los 4 cambios de entrenador en 10 años costaron £32.1M en indemnizaciones, más el costo oculto de ruptura táctica estimado en ~12 puntos de tabla (equivalente a ~£45M en diferencial de Champions League revenue)."
    )
...
with tab3:
    ...
    fig_corr.update_layout(
        **PLOTLY_THEME, height=400, title="Matriz de correlaciones — variables de rendimiento"
    )
    ...
    with col_d1:
        fig_wins = go.Figure()
        for col_w, name, color in [
            ("wins", "Victorias", COLORS["champ"]),
            ("draws", "Empates", COLORS["warn"]),
            ("losses", "Derrotas", COLORS["manutd"]),
        ]:
            fig_wins.add_trace(
                go.Bar(x=df_f["season"], y=df_f[col_w], name=name, marker_color=color, opacity=0.85)
            )
    ...
    with col_d2:
        ...
        fig_reg.update_layout(**PLOTLY_THEME, title="Regresión: Goles a Favor → Puntos", height=320)
...
with tab4:
    ...
    sim_ucl = (
        "Champions League ✅"
        if sim_pts >= 71
        else ("Europa League ⚠️" if sim_pts >= 60 else "Nada 🔴")
    )

    st.markdown("<br>", unsafe_allow_html=True)
    r1, r2, r3, r4 = st.columns(4)
    with r1:
        st.metric("Puntos proyectados", sim_pts, delta=f"{sim_pts - 64:+d} vs baseline (Moyes)")
    with r2:
        st.metric("Posición estimada", f"{sim_pos}°")
    with r3:
        st.metric("Brecha vs campeón", f"{sim_gap} pts")
    with r4:
        st.metric("Clasificación europea", sim_ucl)
